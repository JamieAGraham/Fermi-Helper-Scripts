#!/usr/bin/env python
# Original make3FGLxml.py written by T. Johnson see http://fermi.gsfc.nasa.gov/ssc/data/analysis/user/
# Edited to make whole-sky xml files and region maps by J Graham.
class srcList:
	#arguments are:
	...
	def __init__(self,sources,out='mymodel.xml'):
	...
	def Print(self):
		...
		print 'Selecting %s degrees around (ra,dec)=(%s,%s)' %(self.roi[2],266.405,-28.9362)
...
def cli():
	...
	parser.add_argument("-s","--sig",type=float,default=5.,help="Average significance below which all source parameters are fixed, defaults to 5.  Note, if using the 3FGL catalog xml file as input, this is actually a cut on TS, so adjust accordingly.")
	...
	sL=srcList(args.catalog,args.outputxml)
	sL.makeModel(args.galfile,args.galname,args.isofile,args.isoname,args.normsonly,args.extDir,args.radLim,args.maxRad,args.ExtraRad,args.sig,args.varFree,args.psForce,args.makeRegion,args.GIndexFree,args.edisp,args.writeDir,args.oldNames)
